...
### 支部別リスト（`docs/list/<言語コード>/`）

アプリが支部サイトの言語に応じて読み分ける **3 系統 SCP 報告書フィード**は、リポジトリでは **`docs/list/<code>/`** に置きます（Pages 上の URL は **`list/<code>/…`**）。

| コード | ディレクトリ | 内容（現状） |
|--------|----------------|----------------|
| `jp` | `docs/list/jp/` | 日本支部（`scp-jp.json` / `scp.json` / `scp-int.json`） |
| `ru` / `cn` 等 | （未実装） | 将来、同じレイアウトで追加する想定 |

## アプリ 3 系統フィード: `docs/list/jp/scp-jp.json`

ScpDocs の **`SCPArticleListPayload`** 互換（`schemaVersion: 1`）。**`docs/list/jp/scp-jp.json`** に出力し、Pages では `…/list/jp/scp-jp.json` として配信します（`AppRemoteConfig.scpArticleFeedSiteListCode` とパス接頭辞が対応）。
...
# 既定出力: docs/list/jp/scp-jp.json。一覧は Wikidot、メタは既存 scp_list からマージ（推奨）
...
## アプリ 3 系統フィード: `docs/list/jp/scp.json`（本家メイン和訳）

EN 系統のファイル名は **`scp.json`**（`scp-en.json` ではない）。格納先は **`docs/list/jp/scp.json`**。
...
## アプリ 3 系統フィード: `docs/list/jp/scp-int.json`（国際支部和訳）

`docs/scp_list.json` の **`hubLinkedPaths`**（`/scp-数字-2文字`、`-jp` 除外）をエントリの正とする。出力は **`docs/list/jp/scp-int.json`**。

- **タイトル**: scp-international から辿る各一覧ページをクロールして `<li>` から抽出。取れない場合は **直前コミットの同パス**（`--merge-titles-from`、既定で `docs/list/jp/scp-int.json` が存在すれば使用）を参照し、それでも無い場合は **`SCP-番号-XX`** 形式のフォールバック。
...
| **Update scp-jp.json (app feed)** | **`docs/list/jp/scp-jp.json`**。**毎日 16:30 UTC** ＋手動。`docs/scp_list.json` からメタをマージしつつ Wikidot 一覧を再取得。 |
| **Update scp.json (app feed)** | **`docs/list/jp/scp.json`（本家メイン和訳）**。**毎日 16:45 UTC** ＋手動。 |
| **Update scp-int.json (app feed)** | **`docs/list/jp/scp-int.json`（国際支部パス）**。**毎週月曜 17:00 UTC** ＋手動。`hubLinkedPaths` ＋国際一覧クロール（所要長め）。 |
